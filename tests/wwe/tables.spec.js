...
test.describe('Automation - Working With Table', () => {
  ...
  test('Playwright Test Case - table', async ({ page }) => {
    ...
    //expect(rows.length).to.be.within(1, 50);
   ...
   // or
    ...
    const secondrowvalues = await page.locator('//table[@id="table1"]/tbody/tr/td[2]').allTextContents()

    console.log(secondrowvalues)
   

   const sortedArray = [...secondrowvalues].sort((a, b) => a.localeCompare(b));

   expect(secondrowvalues).toEqual(sortedArray);
